...
namespace rtbot
{
    ...
    public:
        ...
        Buffer(int channelSize_, int windowSize_)
            : data(channelSize_ * windowSize_, T(0)), channelSize(channelSize_), windowSize(windowSize_)
        {
        }
        ...
        void add(const vector<T> &msg)
        {
            if (msg.size() != channelSize)
                throw std::__throw_invalid_argument("Buffer::add() with wrong number of channels");
    ...
    public:
        ...
        void addChildren(Op const &child)
        {
    ...
    public:
        ...
        MA(string id_, Param const &pars_ = {})
            : Op<double>(id_), pars(pars_)
        {}
