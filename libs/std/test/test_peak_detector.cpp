...
SCENARIO("PeakDetector handles basic peak detection", "[PeakDetector]") {
  GIVEN("A PeakDetector with window size 3") {
    auto detector = std::make_shared<PeakDetector>("test", 3);
    auto col = std::make_shared<Collector>("c", std::vector<std::string>{"number"});
    detector->connect(col, 0, 0);
...
SCENARIO("PeakDetector handles edge cases", "[PeakDetector]") {
  ...
  GIVEN("A PeakDetector with window size 5") {
    auto detector = std::make_shared<PeakDetector>("test", 5);
    auto col = std::make_shared<Collector>("c", std::vector<std::string>{"number"});
    detector->connect(col, 0, 0);
...
SCENARIO("PeakDetector handles state serialization", "[PeakDetector][State]") {
  GIVEN("A PeakDetector with processed data") {
    ...
    WHEN("State is serialized and restored") {
      ...
      auto restored = std::make_shared<PeakDetector>("test", 3);
...
SCENARIO("PeakDetector works in a PPG analysis pipeline", "[PeakDetector][Integration]") {
  ...
  init_telemetry();
#endif
  auto input = std::make_shared<Input>("i1", std::vector<std::string>{PortType::NUMBER});
  ...
  // Connect pipeline:
  // input -> ma_short -> minus -> peak -> join -> output -> col
  //      \-> ma_long /                   /
  //      \----------------------------->/

  input->connect(ma_short)->connect(minus, 0, 0)->connect(peak)->connect(join, 0, 0)->connect(output)->connect(col);
