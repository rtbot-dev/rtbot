...
// ---------------------------------------------------------------------------
// SCENARIO — emit_moving_sum parity vs FE MSUM_UPDATE
// ---------------------------------------------------------------------------
SCENARIO("emit_moving_sum matches FE MSUM_UPDATE bit-exactly",
         "[stateful][msum][parity]") {
  ...
  // --- FE reference: INPUT 0, MSUM_UPDATE W, END ---------------------------
  std::vector<double> bc = {
      fused_op::INPUT, 0.0,
      fused_op::MSUM_UPDATE, static_cast<double>(W),
      fused_op::END,
  };
  ...
  // --- JIT path ------------------------------------------------------------
  // Build function:
  //   uint8_t ms_test(double* state, int64_t t, double v,
  //                   int64_t* out_t, double* out_v)
  auto llvm_ctx = std::make_unique<llvm::LLVMContext>();
  auto mod      = std::make_unique<llvm::Module>("ms_test_mod", *llvm_ctx);
  ...
  llvm::Function* fn = llvm::Function::Create(
      fn_ty, llvm::Function::ExternalLinkage, "ms_test", mod.get());
  ...
  StatefulOutput out = emit_moving_sum(ec, /*state_offset=*/0, W, arg_t, arg_v);
  ...
  static rtbot::JitContext jit_ms;
  jit_ms.compile_module(std::move(mod), std::move(llvm_ctx));
  auto ms_fn = jit_ms.lookup<bool (*)(double*, int64_t, double, int64_t*, double*)>("ms_test");
  REQUIRE(ms_fn != nullptr);

  std::vector<double> jit_state(W + 3, 0.0);
  std::vector<std::pair<int64_t, double>> jit_out;
  for (std::size_t i = 0; i < N; ++i) {
    int64_t ot = 0;
    double  ov = 0.0;
    if (ms_fn(jit_state.data(), static_cast<int64_t>(i + 1), values[i], &ot, &ov))
      jit_out.push_back({ot, ov});
  }

  // --- Parity check --------------------------------------------------------
  // W=14, N=500 → 487 emitted outputs (warmup consumes first W-1=13 ticks).
