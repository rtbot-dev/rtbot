...
PYBIND11_MODULE(rtbotapi, m) {
  ...
  m.def("addToMessageBuffer", &addToMessageBuffer);
  m.def("processMessageBuffer", &processMessageBuffer);
  m.def("processMessageBufferDebug", &processMessageBufferDebug);

  m.def("getProgramEntryOperatorId", &getProgramEntryOperatorId);
  m.def("getProgramEntryPorts", &getProgramEntryPorts);
  m.def("getProgramOutputFilter", &getProgramOutputFilter);
