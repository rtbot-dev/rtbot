...
TEST_CASE("Cosine Resampler test emit at right frecuencies")
{
...
TEST_CASE("Hermite Resampler test emit at right frecuencies")
{
    auto i1 = HermiteResampler("i1",99);
    auto i2 = HermiteResampler("i2",99);

    SECTION("emits once")
    {    
        for(int i=0; i< 20; i++) {
            map<string,std::vector<Message<>>> emitted =  i1.receive( Message<>(i*100, i*i) );
            if (i == 0 || i == 1 || i == 2) REQUIRE (emitted.empty());
            else if (i == 3) {
