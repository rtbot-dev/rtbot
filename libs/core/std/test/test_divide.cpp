...
TEST_CASE("Divide joint") {
  ...
  divide.receive(Message<uint64_t, double>(1, 1), "i1");
  divide.receive(Message<uint64_t, double>(2, 6), "i1");
  divide.receive(Message<uint64_t, double>(3, 3), "i1");
  divide.receive(Message<uint64_t, double>(4, 4), "i1");

  emitted = divide.receive(Message<uint64_t, double>(2, 3), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 2);
  REQUIRE(emitted.find("divide")->second.at(0).time == 2);

  emitted = divide.receive(Message<uint64_t, double>(4, 4), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 1);
  REQUIRE(emitted.find("divide")->second.at(0).time == 4);

  emitted = divide.receive(Message<uint64_t, double>(3, 5), "i2");
...
TEST_CASE("Divide joint i2 eager") {
  ...
  emitted = divide.receive(Message<uint64_t, double>(2, 2), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 0.5);
  REQUIRE(emitted.find("divide")->second.at(0).time == 1);

  emitted = divide.receive(Message<uint64_t, double>(3, 4), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 1.5);
  REQUIRE(emitted.find("divide")->second.at(0).time == 2);

  emitted = divide.receive(Message<uint64_t, double>(4, 5), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 0.6);
  REQUIRE(emitted.find("divide")->second.at(0).time == 3);

  emitted = divide.receive(Message<uint64_t, double>(5, 8), "i2");

  REQUIRE(emitted.find("divide")->second.at(0).value == 0.5);
  REQUIRE(emitted.find("divide")->second.at(0).time == 4);

  emitted = divide.receive(Message<uint64_t, double>(6, 2), "i2");
