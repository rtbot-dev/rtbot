...
TEST_CASE("simple peak detector") {
  ...
  SECTION("one peak") {
    for (int i = 0; i < 10; i++) op.receive(Message<double>(i, 5 - fabs(1.0 * i - 5)));
    REQUIRE(msg_l.size() == 1);
    REQUIRE(msg_l[0] == Message<double>(5, 5.0));
  }

  SECTION("two peaks") {
    for (int i = 0; i < 14; i++) op.receive(Message<double>(i, i % 5));
...
TEST_CASE("ppg peak detector") {
  ...
  // process the data
  for (auto i = 0u; i < s.ti.size(); i++) i1.receive(Message<double>(s.ti[i], s.ppg[i]));
}
